        ...
        pcmStream.on('end', async () => {
            ...
            try {
                ...
                response.data.on('data', (chunk) => {
                    buffer_data += chunk.toString();
                    const lines = buffer_data.split('\n');
                    buffer_data = lines.pop();

                    for (const line of lines) {
                        if (!headerParsed && line.includes('---AUDIO---')) {
                            headerParsed = true;
                            // Parse header JSON
                            const headerJson = buffer_data.split('---AUDIO---')[0];
                            try {
                                const parsed = JSON.parse(buffer_data.split('\n---AUDIO---')[0]);
                                console.log(`\n--- ARGUMENT LOG ---`);
